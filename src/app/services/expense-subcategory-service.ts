...
export class ExpenseSubcategoryService {
  ...
  listByCategory(idCategory: number): Observable<ExpenseSubcategory[]> {
    ...
    const detalles = JSON.parse(localStorage.gasto_detalle);
  ...
  create(subcategory: ExpenseSubcategory): Observable<ExpenseSubcategory> {
    const subcategories: ExpenseSubcategory[] = JSON.parse(localStorage.gasto_subcategorias);
    const newsubcategory: ExpenseSubcategory = subcategory;
